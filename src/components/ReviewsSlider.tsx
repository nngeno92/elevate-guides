...
export default function ReviewsSlider({ reviews }: ReviewsSliderProps) {
  ...
  const scrollToReview = (direction: 'prev' | 'next') => {
    ...
    const cardWidth = 400; // Approximate width of each review card
      ...
      <div className="relative">
        ...
        >
          {reviews.map((review, index) => (
            <div
              key={index}
              className="flex-shrink-0 w-96 bg-white border border-gray-200 rounded-lg p-6 shadow-sm hover:shadow-md transition-shadow duration-200"
            >
